...
bic_winner_on_screened <- function(A_screen, dat) {
  forms <- lapply(A_screen, function(a) {
    rhs <- paste0("X", a, collapse = " + ")
    as.formula(paste("y ~", rhs))
  })
  ...
  # Modell-Label wie bisher
  paste0("{", paste(A_screen[[j_star]], collapse = ","), "}")
}
...
# BIC-Referenz über alle m replizieren
mc_bic_rep <- tidyr::expand_grid(
  m = sort(unique(mc_res$m)),
  model = mc_bic_full$model
) %>%
  left_join(
    mc_bic_full %>% select(model, win_prob_over_R),
    by = "model"
  ) %>%
  mutate(method = "BIC")
